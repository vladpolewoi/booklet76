...
export async function POST(req: Request) {
  ...
  const sheetCount = parseInt(req.headers.get('X-Sheet-Count') || '5', 10)
  const pdfDoc = await PDFDocument.load(pdfBytes)

  // Create a new PDF with the first two pages
  const newPDF = await PDFDocument.create()

  const booklet = shuffle(pdfDoc.getPageCount(), sheetCount)
...
function shuffle(count: number, sheetCount: number) {
  const arr = Array.from(Array(count).keys()).map((i) => i)
  const PAGES_PER_SHEET = 4
  const PAGES_COUNT = sheetCount * PAGES_PER_SHEET
